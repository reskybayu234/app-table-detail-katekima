...
onMounted(async () => {
  try {
    ...
    const data = await berryStore.fetchProduct(productId);
    product.value = { ...data };
  } catch (error) {
...
const submitForm = async () => {
  try {
    await berryStore.updateProduct(productId, product.value);
    showAlert.value = true;
    alertMessage.value = t("form.successEdit");
...
<template>
  <div class="flex">
    <div class="flex-1 p-6">
      ...
      <h1 class="text-2xl font-bold mb-6">{{ t("form.editTitle") }}</h1>

      <div class="bg-white rounded-lg shadow p-6">
        <SkeletonLoader v-if="isLoading || berryStore.loading" type="form" />
